...
try:
    ...
    print("Warning: syntax_validator.py not found. Syntax validation disabled.")
    syntax_validator_available = False
    SyntaxValidator = None

# Import file deduplication system
try:
    from file_deduplication_system import FileDeduplicationSystem
    file_deduplication_available = True
except ImportError:
    print("Warning: file_deduplication_system.py not found. File deduplication disabled.")
    file_deduplication_available = False
    FileDeduplicationSystem = None
...
class ProjectManager:
    ...
    def __init__(self):
        ...
        # Initialize syntax validator if available
    ...
    async def create_project(self, project_id: str, generated_code: Dict,
        ...
        # Process and write files with deduplication
        files = generated_code.get("files", [])
        valid_files = []
        validation_issues = []
        
        # Initialize file deduplication system
        deduplication_system = FileDeduplicationSystem(project_path) if file_deduplication_available else None
        
        # Prepare files to be written
        files_to_write = {}
        
        # First pass: collect all files and fix paths
        for file_info in files:
            ...
            files_to_write[fixed_path] = content
        
        # Use deduplication system to ensure unique files
        if deduplication_system:
            print(f"[PROJECT MANAGER] Using file deduplication system")
            files_to_write = deduplication_system.ensure_unique_files(files_to_write)
            print(f"[PROJECT MANAGER] After deduplication: {len(files_to_write)} files to write")
        else:
            print(f"[PROJECT MANAGER] File deduplication system not available, using basic duplicate detection")
            ...
            for file_path, content in files_to_write.items():
                ...
                if filename in filename_to_paths:
                    ...
                    validation_issues.append(f"Duplicate filename '{filename}': {file_path} (kept {filename_to_paths[filename]})")
                    continue
                
                filename_to_paths[filename] = file_path
                deduplicated_files[file_path] = content
            
            files_to_write = deduplicated_files

        # Second pass: validate and write deduplicated files
        for fixed_path, content in files_to_write.items():
            # Validate content
            ...
            if is_valid or len(issues) == 0:  # Allow files with warnings
                ...
                # Fix escaped newlines before writing
                if isinstance(content, str):
                    content = content.replace("\\n", "\n")
                    content = content.replace("\\r", "\r")
                    content = content.replace("\\t", "\t")
                
                # Run syntax validation before writing
                if self.syntax_validator and fixed_path.endswith('.swift'):
                    syntax_valid, syntax_errors = self.syntax_validator.validate_syntax(fixed_path, content)
                    if not syntax_valid:
                        print(f"[SYNTAX VALIDATOR] Found {len(syntax_errors)} syntax errors in {fixed_path}")
                        for error in syntax_errors[:3]:  # Show first 3 errors
                            print(f"  - {error}")
                        
                        # Try to auto-fix common issues
                        fixed_content = self.syntax_validator.fix_common_issues(content)
                        if fixed_content != content:
                            print(f"[SYNTAX VALIDATOR] Applied automatic fixes to {fixed_path}")
                            content = fixed_content
                            
                            # Re-validate after fixes
                            syntax_valid, syntax_errors = self.syntax_validator.validate_syntax(fixed_path, content)
                            if not syntax_valid:
                                validation_issues.extend([f"Syntax: {err}" for err in syntax_errors])
    ...
    def get_project_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for root, dirs, files in os.walk(project_path):
                for file in files:
                    if file.endswith('.swift'):
                        swift_files.append(os.path.relpath(os.path.join(root, file), project_path))
            ...
            return {
                ...
                'has_xcodeproj': any(f.endswith('.xcodeproj') for f in os.listdir(project_path))
            }
        except Exception as e:
            return {
                'exists': True,
                'error': f'Error reading project status: {str(e)}'
            }