...
class ComprehensiveCodeValidator:
    ...
    def __init__(self, ios_target: str = "16.0"):
        ...
        self.ios_version_features = {
            17: [
                ...
                '.defaultScrollAnchor(',
                '.scrollBounceBehavior(',
                '.onChange(of:initial:',  # New onChange signature
                'TipKit',
                'SwiftData',
                '@Model',
                'ModelContainer',
        ...
        # iOS 16 specific patterns (things that are OK in iOS 16)
        self.ios_16_patterns = {
            # Navigation
            'NavigationStack': 'iOS 16+ navigation (OK)',
            'NavigationPath': 'iOS 16+ navigation (OK)',
            '.navigationDestination(': 'iOS 16+ navigation (OK)',
            
            # Layout
            'Grid': 'iOS 16+ layout (OK)',
            'GridRow': 'iOS 16+ layout (OK)',
            'Layout': 'iOS 16+ custom layout (OK)',
            'ViewThatFits': 'iOS 16+ adaptive layout (OK)',
            
            # Forms and Controls
            'LabeledContent': 'iOS 16+ form control (OK)',
            '.scrollDismissesKeyboard(': 'iOS 16+ keyboard handling (OK)',
            
            # Charts
            'Chart': 'iOS 16+ Charts framework (OK)',
            
            # Other iOS 16 features
            'ShareLink': 'iOS 16+ sharing (OK)',
            'PhotosPicker': 'iOS 16+ photo picker (OK)',
            '.monospaced(': 'iOS 16+ text styling (OK)',
            '.fontDesign(': 'iOS 16+ font design (OK)',
            '.fontWidth(': 'iOS 16+ font width (OK)',
            'ImageRenderer': 'iOS 16+ image rendering (OK)',
            '.presentationDetents(': 'iOS 16+ sheet presentation (OK)',
            '.presentationDragIndicator(': 'iOS 16+ sheet indicator (OK)',
            '.presentationCornerRadius(': 'iOS 16+ sheet styling (OK)',
            '.presentationBackground(': 'iOS 16+ sheet background (OK)',
            '.interactiveDismissDisabled(': 'iOS 16+ dismiss control (OK)',
            '.requestReview(': 'iOS 16+ StoreKit review (OK)',
            '.scrollContentBackground(': 'iOS 16+ scroll styling (OK)',
            '.toolbarBackground(': 'iOS 16+ toolbar styling (OK)',
            '.toolbarColorScheme(': 'iOS 16+ toolbar theming (OK)',
            'Gauge': 'iOS 16+ gauge view (OK)',
            '.gauge': 'iOS 16+ gauge style (OK)',
            'Table': 'iOS 16+ table view (OK)',
            '.tableStyle(': 'iOS 16+ table styling (OK)',
            '.searchable(': 'iOS 16+ search (OK with correct signature)',
            '.searchScopes(': 'iOS 16+ search scopes (OK)',
            'AnyLayout': 'iOS 16+ layout switching (OK)'
        }
    ...
    def _check_ios_features(self, file_path: str, content: str) -> List[CodeIssue]:
        ...
        # Check for features that require newer iOS versions
        for ios_version, features in self.ios_version_features.items():
            if self.ios_major < ios_version:
                for feature in features:
                    if feature in content:
                        # Special handling for onChange which has different signatures
                        if feature == '.onChange(of:initial:' and '.onChange(of:' in content:
                            # Check if it's the iOS 17+ version with initial parameter
                            onChange_pattern = r'\.onChange\(of:[^)]+initial:'
                            if re.search(onChange_pattern, content):
                                issues.append(CodeIssue(
                                    severity='error',
                                    category='ios_version',
                                    file_path=file_path,
                                    line_number=None,
                                    message=f"'.onChange(of:initial:)' requires iOS 17+, use '.onChange(of:)' for iOS 16",
                                    fix="Remove 'initial:' parameter from onChange"
                                ))
                        else:
                            issues.append(CodeIssue(
                                severity='error',
                                ...
                                fix=f"Remove {feature} or use iOS {self.ios_major} alternative"
                            ))
        
        # Additional iOS 16 specific checks
        if self.ios_major == 16:
            # Check for incorrect onChange usage
            onChange_new_pattern = r'\.onChange\(of:\s*\w+\)\s*\{\s*(?:oldValue|_)\s*,\s*newValue'
            if re.search(onChange_new_pattern, content):
                issues.append(CodeIssue(
                    severity='error',
                    category='ios_version',
                    file_path=file_path,
                    line_number=None,
                    message="iOS 16 onChange takes only newValue parameter, not (oldValue, newValue)",
                    fix="Use { newValue in ... } for iOS 16, not { oldValue, newValue in ... }"
                ))
            
            # Check for NavigationView usage (deprecated in iOS 16)
            if 'NavigationView' in content and 'NavigationStack' not in content:
                issues.append(CodeIssue(
                    severity='warning',
                    category='deprecated_api',
                    file_path=file_path,
                    ...
                    fix="Use NavigationStack for iOS 16+"
                ))
            
            # Check for proper async image usage
            if 'AsyncImage' in content:
                # Ensure proper error handling
                if not re.search(r'AsyncImage[^}]*\.placeholder', content):
                    issues.append(CodeIssue(
                        severity='warning',
                        category='best_practice',
                        file_path=file_path,
                        line_number=None,
                        message="AsyncImage should include placeholder for better UX",
                        fix="Add .placeholder { ProgressView() } to AsyncImage"
                    ))
    ...
    def _check_ios16_best_practices(self, file_path: str, content: str) -> List[CodeIssue]:
        """Check for iOS 16 best practices and modern patterns"""
        issues = []
        
        # Check for completion handler patterns (should use async/await)
        completion_pattern = r'completion:\s*@escaping\s*\([^)]*\)\s*->\s*Void'
        if re.search(completion_pattern, content):
            issues.append(CodeIssue(
                ...
                file_path=file_path,
                line_number=None,
                message='Use async/await instead of completion handlers for iOS 16+',
                fix='Convert completion handlers to async functions'
            ))
        
        # Check for DispatchQueue.main usage (should use @MainActor)
        if 'DispatchQueue.main' in content:
            issues.append(CodeIssue(
                severity='warning',
                category='modernization',
                file_path=file_path,
                line_number=None,
                message='Use @MainActor instead of DispatchQueue.main for iOS 16+',
                fix='Replace DispatchQueue.main.async with @MainActor or MainActor.run'
            ))
        
        # Check for .foregroundColor (deprecated)
        if '.foregroundColor(' in content:
            issues.append(CodeIssue(
                severity='warning',
                category='deprecated_api',
                file_path=file_path,
                line_number=None,
                message='.foregroundColor is deprecated in iOS 16+',
                fix='Use .foregroundStyle instead'
            ))
        
        # Check for proper NavigationStack usage
        if 'NavigationStack' in content:
            # Check if using old NavigationLink style
            old_nav_pattern = r'NavigationLink\(destination:'
            if re.search(old_nav_pattern, content):
                issues.append(CodeIssue(
                    ...
                    file_path=file_path,
                    line_number=None,
                    message='Use value-based NavigationLink with .navigationDestination for NavigationStack',
                    fix='Use NavigationLink(value:) with .navigationDestination(for:destination:)'
                ))
        
        # Check for @ObservedObject in iOS 16 (prefer @StateObject)
        observed_pattern = r'@ObservedObject\s+var'
        if re.search(observed_pattern, content) and 'View' in content:
            issues.append(CodeIssue(
                severity='warning',
                category='best_practice',
                file_path=file_path,
                line_number=None,
                message='Consider using @StateObject instead of @ObservedObject for view-owned objects',
                fix='Use @StateObject for objects created and owned by the view'
            ))
        
        # Check for environment dismiss usage
        if '@Environment(\\.presentationMode)' in content:
            issues.append(CodeIssue(
                ...
                file_path=file_path,
                line_number=None,
                message='@Environment(\\.presentationMode) is deprecated in iOS 16+',
                fix='Use @Environment(\\.dismiss) instead'
            ))
        
        # Check for proper error handling in async functions
        async_func_pattern = r'func\s+\w+[^{]*async[^{]*\{'
        if re.search(async_func_pattern, content):
            # Check if the function has try/catch
            if 'try await' in content and 'catch' not in content:
                issues.append(CodeIssue(
                    ...
                    file_path=file_path,
                    line_number=None,
                    message='Async functions with try await should include proper error handling',
                    fix='Add do-catch block or mark function as throws'
                ))
        
        # Check for SF Symbols usage without fallback
        sf_symbol_pattern = r'Image\(systemName:\s*"[^"]+"\)'
        if re.search(sf_symbol_pattern, content):
            # Check if there's a renderingMode or fallback
            if '.renderingMode' not in content and '.symbolRenderingMode' not in content:
                issues.append(CodeIssue(
                    severity='info',
                    category='best_practice',
                    file_path=file_path,
                    line_number=None,
                    message='Consider adding .symbolRenderingMode for SF Symbols in iOS 16+',
                    fix='Add .symbolRenderingMode(.monochrome) or .symbolRenderingMode(.hierarchical)'
                ))
