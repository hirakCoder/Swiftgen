...
try:
    from component_reference_validator import ComponentReferenceValidator
    component_validator = ComponentReferenceValidator()
except ImportError:
    print("Warning: component_reference_validator.py not found. Component reference validation disabled.")
...
try:
    ...
    print("Warning: performance_optimizer.py not found. Performance optimization disabled.")
except Exception as e:
    ...
    for pattern in patterns[1:]:  # Skip the first pattern since we already checked it
        ...
        if match:
            ...
            if words:
                ...
                if len(app_name) > 50:
                    app_name = app_name[:50].rsplit(' ', 1)[0]
...
async def _generate_app_async(project_id: str, request: GenerateRequest):
    ...
    try:
        ...
        log_info(Component.GENERATION, "generation_started", f"Starting app generation for project {project_id}", project_id, 
                details={"description": request.description, "ios_version": request.ios_version})
        
        # Start project tracking
        logger.start_project_tracking(project_id, request.app_name or "Unknown", request.description)
        
        # Extract app name using LLM first, fallback to regex
        with LogTimer(Component.GENERATION, "app_name_extraction", "app name extraction", project_id):
            if request.app_name and request.app_name != "MyApp":
                app_name = request.app_name
                log_info(Component.GENERATION, "app_name_provided", f"Using provided app name: {app_name}", project_id)
            else:
                # Try LLM extraction first
                llm_app_name = await enhanced_service.extract_app_name(request.description)
                if llm_app_name:
                    app_name = llm_app_name
                    log_info(Component.GENERATION, "app_name_extracted_llm", f"LLM extracted app name: {app_name}", project_id)
                else:
                    # Fallback to regex extraction
                    app_name = extract_app_name_from_description(request.description)
                    log_info(Component.GENERATION, "app_name_extracted_regex", f"Regex extracted app name: {app_name}", project_id)

        log_info(Component.GENERATION, "app_name_determined", f"Final app name: {app_name}", project_id, 
                details={"app_name": app_name, "description": request.description})
        
        # Update project tracking
        logger.update_project_status(project_id, "extracting_name", "app_name_extraction")
        ...
        if performance_optimizer:
            cached_generation = performance_optimizer.get_cached_generation(request.description)
            if cached_generation:
                print(f"[PERFORMANCE] Using cached generation for faster response")
                # Update app name in cached generation
                cached_generation["app_name"] = app_name
            ...
            # Detect app type for optimization
            detected_app_type = performance_optimizer.detect_app_type(request.description)
            if detected_app_type:
                print(f"[PERFORMANCE] Detected {detected_app_type} app type")
        else:
            skip_generation = False
        ...
        # Initialize architect variable at the top level
        architect = None
        ...
        if is_complex_app:
            ...
            try:
                ...
                architect = None  # Ensure architect is None if import fails
...
Important: Return ONLY valid JSON, no explanatory text."""
        ...
        if "app_type" not in generated_code and architect is not None and hasattr(architect, 'identify_app_type'):
        ...
        if performance_optimizer and not skip_generation:
            performance_optimizer.cache_generation(request.description, generated_code)
        ...
        if performance_optimizer and "files" in generated_code:
            generated_code["files"] = performance_optimizer.optimize_file_generation(generated_code["files"])
        
        # Component reference validation
        if component_validator and "files" in generated_code:
            # Skip validation for simple template apps if performance mode
            if performance_optimizer and detected_app_type and performance_optimizer.should_skip_validation(detected_app_type, app_complexity):
                print(f"[PERFORMANCE] Skipping component validation for {detected_app_type} template app")
            else:
                is_valid, missing_components = component_validator.validate_references(generated_code["files"])
                if not is_valid:
                    print(f"[MAIN] Component reference validation found missing components:")
                    for component_info in missing_components:
                        if component_info:
                            print(f"  - {component_info.get('name', 'Unknown')} ({component_info.get('type', 'Unknown')})")
                    # NOTE: We don't automatically fix missing components during generation
                    # as this can create unnecessary files for Swift standard types
                    # Only log the validation result for now
        ...
        # Step 2: Run quality assurance (with parallel validation if available)
        await notify_clients(project_id, {
            "type": "status",
        ...
        # Use parallel validation if performance optimizer is available
        if performance_optimizer and hasattr(performance_optimizer, 'parallel_validate'):
            validators = []
            if comprehensive_validator:
                validators.append(comprehensive_validator)
            if component_validator:
                validators.append(component_validator)
            
            if validators:
                parallel_result = await performance_optimizer.parallel_validate(
                    generated_code.get("files", []),
                    validators
                )
                print(f"[PERFORMANCE] Parallel validation completed in {parallel_result['time']:.2f}s")
...
async def generate_app(request: GenerateRequest):
    ...
    try:
        # Extract app name using LLM or fallback
        if request.app_name and request.app_name != "MyApp":
            app_name = request.app_name
        else:
            # Try LLM extraction first with timeout
            try:
                llm_app_name = await asyncio.wait_for(
                    enhanced_service.extract_app_name(request.description),
                    timeout=5.0
                )
                app_name = llm_app_name if llm_app_name else extract_app_name_from_description(request.description)
        ...
        # Start async generation with proper error handling
        asyncio.create_task(_generate_app_with_error_handling(project_id, request, app_name))
        
        # Return project info for WebSocket connection
        return {
            "project_id": project_id,
            "app_name": app_name,
            "status": "generating",
        ...
        logger.error(f"Error in generate_app: {str(e)}")
        generation_stats["failed_generations"] += 1
        raise HTTPException(status_code=500, detail=str(e))


async def _generate_app_with_error_handling(project_id: str, request: GenerateRequest, app_name: str):
    """Wrapper for async generation with proper error handling"""
    try:
        await _generate_app_async(project_id, request)
    except Exception as e:
        ...
        if project_id in project_contexts:
            project_contexts[project_id]["status"] = "failed"
            project_contexts[project_id]["error"] = str(e)
...
async def modify_app(request: ModifyRequest):
    ...
    try:
        ...
        if project_id not in project_state:
            # Read current files from project
            current_files = []
            ...
            current_state = project_state[project_id]
...
async def get_generation_stats():
    ...
    return {
        ...
        "performance_metrics": performance_optimizer.get_performance_report() if performance_optimizer else {}
    }
...
async def get_comprehensive_monitoring():
    ...
    # Get comprehensive logger metrics
    logger_metrics = logger.get_metrics()
    
    # Get WebSocket connection stats
    websocket_stats = ws_monitor.get_connection_stats()
    
    # Get active projects
    active_projects = logger.get_active_projects()
    ...
    system_health = {
        "timestamp": datetime.now().isoformat(),
        "services": {
            "enhanced_llm": len(enhanced_service.get_available_models()) if hasattr(enhanced_service, 'get_available_models') else 0,
            "rag_kb": rag_knowledge_base is not None,
            "simulator": simulator_service is not None,
        ...
        "performance": {
            "total_events": logger_metrics.get("total_events", 0),
            "error_rate": (logger_metrics.get("error_count", 0) / max(logger_metrics.get("total_events", 1), 1)) * 100,
...
@app.get("/api/monitoring/project/{project_id}")
async def get_project_monitoring(project_id: str):
    """Get detailed monitoring for specific project"""
    
    # Get project logs
    project_logs = logger.get_project_logs(project_id)
    
    # Get WebSocket connection info
    websocket_info = ws_monitor.get_project_connection_info(project_id)
    
    # Get project context
    project_context = project_contexts.get(project_id, {})
